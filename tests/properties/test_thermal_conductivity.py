...
def test_KLMX2_li():
    ...
    k = [0.1383, 0.2069]
    km = KLMX2_Li(w, k, rho)
    assert isclose(km, 0.167, rtol=1e-2)
...
def test_KVPC_stiel_thodos():
    ...
    assert isclose(kpc, 1.78e-2, rtol=1e-2)
...
def test_KVMXPC_stiel_thodos():
    ...
    assert isclose(kpc, 1.50e-2, rtol=1e-2)
...
def test_KVMX2_wassilijewa():
    ...
    assert isclose(k_mix, 1.92e-2, rtol=5e-2)
