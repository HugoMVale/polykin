...
def test_input_validation():
    ...
    with pytest.raises(ValueError):
        _ = Eyring([1e3, 2e3], [-1e3, 2e3], [0.5, 1])
    with pytest.raises(ValueError):
        _ = Eyring([1e3, 2e4], [1e3, 2e3], [2, 0.5])
    with pytest.raises(ValueError):
        _ = Eyring([1e3, 2e4], [1e3, 2e3], [0.4, 0.5, 0.8])
    with pytest.raises(ValueError):
...
def test_shape_validation():
    ...
    with pytest.raises(ShapeError):
        _ = Arrhenius(k0=[1.0, 1.0], EaR=1, T0=1.0)
    with pytest.raises(ShapeError):
        _ = Arrhenius([1, 2], [1, 2, 3], 298)
    with pytest.raises(ShapeError):
        _ = Arrhenius(k0=1.0, EaR=[1.0, 1.0], T0=1.0)
    with pytest.raises(ShapeError):
        _ = Arrhenius(k0=1.0, EaR=1.0, T0=[1.0, 1.0])
...
def test_division_Arrhenius_Arrhenius_scalar():
    k1 = Arrhenius(1e2, 1e4, T0=340, Tmin=300, Tmax=380, name="k1")
    k2 = Arrhenius(2e2, 5e3, T0=360, Tmin=320, Tmax=400, name="k2")
    T = 350.0
    k1_value = k1(T, "K")
...
def test_product_Arrhenius_Arrhenius_array():
    k1 = Arrhenius(
        [1e2, 2e3],
        [2e4, 1e4],
        T0=[340, 341],
