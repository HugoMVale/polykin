...
def test_mu_Carreau_Yasuda():
    ...
    assert isclose(mu_Carreau_Yasuda(1e6, mu0, muinf, lmbda, n, a),
                   1e-3, rtol=0.1)
    assert isclose(mu_Carreau_Yasuda(1e2, mu0, muinf, lmbda, n, a),
                   2.5e-2, rtol=0.1)
...
def test_mu_Cross():
    ...
    assert isclose(
        mu_Cross(10.0, mu0, lmbda, n),
        mu_Cross_modified(10.0, mu0, lmbda/mu0, n))
...
def test_aT_WLF():
    ...
    isclose(aT_WLF(-20 + 273.15, Tg)/aT_WLF(25 + 273.15, Tg)*10,
            5140, rtol=1e-2)
