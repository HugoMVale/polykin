...
from polykin.hmt.diffusion import (
...
def test_uptake_constc_sheet():
    ...
    assert isclose(uptake_constc_sheet(0.25 + 1e-7), uptake_constc_sheet(0.25 - 1e-7))
...
def test_uptake_constc_sphere():
    ...
    assert isclose(uptake_constc_sphere(0.25 - 1e-7), uptake_constc_sphere(0.25 + 1e-7))
    # "Seeder and Henley, p. 123"
...
def test_diffusivity_composite():
    for Dd, Dc in [(1, 2), (2, 1)]:
        ...
        assert isclose(diffusivity_composite(Dd, Dc, fd=0.5), (Dd + Dc) / 2, rtol=0.1)


def test_uptake_convection_sheet():
    assert isclose(uptake_convection_sheet(0, 1), 0.0)
    assert isclose(uptake_convection_sheet(1e2, 1), 1.0)
    ...
    assert isclose(uptake_convection_sheet(0.25, 1e6), uptake_constc_sheet(0.25))


def test_uptake_convection_sphere():
    assert isclose(uptake_convection_sphere(0, 1), 0.0)
    assert isclose(uptake_convection_sphere(1e2, 1), 1.0)
    ...
    assert isclose(uptake_convection_sphere(0.25, 1e6), uptake_constc_sphere(0.25))
