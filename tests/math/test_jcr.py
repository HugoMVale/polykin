...
def test_confidence_ellipse():
    ...
    alpha = 0.05
    color = "black"
    confidence_ellipse(ax, center, cov, ndata, alpha=alpha, color=color)
...
def test_confidence_region():
    ...
    beta = (0.15, -0.05)
    Y = model(X, beta) + np.random.normal(0.0, 0.05, len(X))

    def sse(beta) -> float:
        Ye = model(X, beta)
        return sum((Ye - Y) ** 2)

    sol = minimize(sse, (-1.0, 1.0))
    beta_est = sol.x

    jcr = confidence_region(beta_est, sse=sse, ndata=ndata)
    assert len(jcr[0]) > 1
