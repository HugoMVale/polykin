...
def test_inst_copolymer_multicomponent():
    ...
    assert allclose(F_t, F_m)  # type: ignore
...
def test_monomer_drift_multi():
    ...
    for f10 in [0.2, 0.5, 0.9]:
        ...
        sol_m = monomer_drift_multi([f10, 1.0 - f10], x, r)
        assert allclose(sol_b1, sol_b2, atol=1e-3)
        assert allclose(sol_b2, sol_m[:, 0], atol=1e-4)
    with pytest.raises(ShapeError):
        _ = monomer_drift_multi([f10, 1.0 - f10], x, np.ones((3, 3)))
...
def test_convert_Qe():
    ...
    assert allclose(r.diagonal(), [1.0, 1.0, 1.0])
    assert allclose(r[0, :], [1.0, 0.5, 40.0], rtol=0.1)
...
def test_transitions_multi():
    ...
    assert allclose(P.diagonal(), [0.241, 0.295, 0.209], rtol=1e-2)
    assert allclose(P.sum(axis=-1), np.ones(3))
...
def test_transitions_multi_2():
    ...
    for f1 in [0.2, 0.5, 0.9]:
        ...
        sol_m = transitions_multi([f1, 1.0 - f1], r)
        for i in range(2):
            for j in range(2):
...
def test_sequence_multi():
    ...
    for f1 in [0.2, 0.5, 0.9]:
        ...
        assert allclose(sol_b, sol_m)  # type: ignore
        # SLD
        sol_b = m.sequence(f1, k)
        sol_m = sequence_multi(P.diagonal(), k)
        assert allclose(sol_b["1"], sol_m[0])
...
def test_radical_fractions_ternary():
    ...
    f1 = 0.3
    f2 = 1 - f1
    p1, p2, p3 = radical_fractions_ternary(f1, f2, k12, k21, k13, k31, k23, k32)
    p1_sol = k21 * f1 / (k21 * f1 + k12 * f2)
    assert allclose([p1, p2, p3], [p1_sol, 1 - p1_sol, 0])  # type: ignore
    # 1 + 3
    f1 = 0.3
    f2 = 0.0
    f3 = 1 - f1 - f2
    ...
    assert allclose([p1, p2, p3], [p1_sol, 0, 1 - p1_sol])  # type: ignore
    # 2 + 3
    f1 = 0
    f2 = 0.4
    f3 = 1 - f1 - f2
    ...
    assert allclose([p1, p2, p3], [0, p1_sol, 1 - p1_sol])  # type: ignore
...
def test_radical_fractions_multi():
    ...
    assert allclose(p_multi, p_ternary)  # type: ignore
