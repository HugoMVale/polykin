...
def dist2():
    return [
        LogNormal(80, 3.0, 50, name="LogNormal"),
    ...
    for d in distributions:
        for kind in ["number", "mass", "gpc"]:
            pdf_integral, atol = integrate.quad(
                lambda x: d.pdf(x, kind=kind), 0, np.inf
            )
...
def test_data_distribution():
    ...
    d2 = DataDistribution(length_data, pdf_data, kind=kind, M0=d1.M0, name="data")
    rtol = 1e-4
...
def test_fit_itself(dist1, dist2):
    ...
    for d in distributions:
        ...
        d2 = DataDistribution(
            length_data, pdf_data, kind=kind, M0=d.M0, name="data" + d.name
        )
        dfit = d2.fit(type(d), 1)
        for attr in ["DPn", "DPw", "DPz", "Mn", "Mw", "Mz", "PDI", "M0"]:
            assert isclose(getattr(dfit, attr), getattr(d, attr), rtol=rtol)


def test_plot_method(dist1):
    _ = dist1[0].plot()
    for kind in ["number", "mass", "gpc"]:
        _ = dist1[0].plot(kind=kind)
    for cdf in [0, 1, 2]:
        _ = dist1[0].plot(cdf=cdf)
