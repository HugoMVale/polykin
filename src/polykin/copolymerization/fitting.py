...
from polykin.copolymerization.binary import inst_copolymer_binary, monomer_drift_binary
from polykin.copolymerization.copodataset import (
    CopoDataset_Ff,
    CopoDataset_Fx,
    CopoDataset_fx,
)
...
class CopoFitResult:
    ...
    def __repr__(self):
        """Return a string representation of the copolymerization fit result."""
        s1 = (
...
def fit_copo_data(
    ...
    * [`fit_Finemann_Ross`](fit_Finemann_Ross.md): alternative method.
    ...
    # Fit data
    if method == "NLLS":
        r_opt, cov, sse = _fit_copo_NLLS(data_Ff, data_fx, data_Fx, r_guess, ndata)
    elif method == "ODR":
    ...
    if ndata > npar and cov is not None:
        ...
        ci_r = se_r * tdist.ppf(1.0 - alpha / 2, ndata - npar)
    else:
        se_r = [np.nan, np.nan]
        ci_r = [np.nan, np.nan]
    ...
    if plot_data:
        ...
        if data_Ff:
            ...
            f1 = np.linspace(0.0, 1.0, npoints)
        ...
        if data_fx:
            ...
            ax.set_xlim(0.0, 1.0)
            ax.set_ylim(0.0, 1.0)
            x = np.linspace(0.0, xmax, npoints)
            for dataset in data_fx:
        ...
        ax.set_xlim(0.0, 1.0)
        ax.set_ylim(0.0, 1.0)
        ax.plot([0.0, 1.0], [0.0, 1.0], color="black", linewidth=0.5)
        for dataset in data_Ff:
...
def _fit_copo_NLLS(
    ...
    def sse(r: tuple[float, float], atol=1e-4, rtol=1e-4) -> float:
        ...
        # f1(x, f10) datasets
        for dataset in data_fx:
            f1_est = monomer_drift_binary(
                dataset.f10, dataset.x, *r, atol=atol, rtol=rtol
            )
            rx = (dataset.f1 - f1_est) / dataset.scale_f1
            result += dataset.weight * dot(rx, rx)
        ...
        for dataset in data_Fx:
            f10 = dataset.f10
            x = dataset.x
            f1_est = monomer_drift_binary(f10, x, *r, atol=atol, rtol=rtol)
            F1_est = f1_est + (f10 - f1_est) / (x + 1e-10)
            rx = (dataset.F1 - F1_est) / dataset.scale_F1
            result += dataset.weight * dot(rx, rx)
        return result
