...
def fixpoint_damped(
    ...
    *,
    ...
    Direct substitution with relaxation is a damped fixed-point iteration
    where the next iterate is obtained from a convex combination of the current
    iterate and its direct substitution update according to:

    $$ \mathbf{x}_{k+1} = \omega \mathbf{g}(\mathbf{x}_k) + (1 - \omega) \mathbf{x}_k $$

    where $0 < \omega \leq 1$ is the relaxation parameter. When $\omega=1$,
    the method is equivalent to standard direct substitution. For $\omega<1$,
    the update is damped, which can improve robustness for mildly unstable
    ...
    * [`fixpoint_wegstein`](fixpoint_wegstein.md):
    ...
    >>> sol = fixpoint_damped(g, x0=np.array([0.0, 0.0]), omega=0.8)
    >>> print(f"x = {sol.x}")
